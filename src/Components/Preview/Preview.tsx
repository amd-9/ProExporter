...
const getTestData = async (config: any) => {
  const { currentProject, context } = config;

  const testPlanClient = getClient(TestPlanRestClient);
  const testSuites: PagedList<TestSuite> = await testPlanClient.getTestSuitesForPlan(
    currentProject.id,
    context.plan.id,
  );
  ...
  for (const suite of testSuites) {
    const suiteTestCases: PagedList<TestCase> = await testPlanClient.getTestCaseList(
      currentProject.id,
      context.plan.id,
      suite.id,
    );
    ...
    const casesWithSteps = suiteTestCases.reduce((acc, { workItem }) => {
      ...
      const {
        steps: { step: stepsData },
      } = xml2js(stepsField['Microsoft.VSTS.TCM.Steps'], { compact: true }) as any;
...
const Preview = () => {
  ...
  useEffect(() => {
    if (state === PreviewState.Loading) {
      (async () => {
        setSuitesData(await getTestData(config));
  ...
  const getStepsTableItemsProvider = (steps: any) => new ArrayItemProvider<ITestCaseStepItem>(steps);
  ...
  return (
    <div>
      ...
      <ButtonGroup className="flex-wrap">
        <Button primary text="Save as Excel" onClick={showConfig} />
